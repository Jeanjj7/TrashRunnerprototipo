...
class Obstaculo(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.image = pygame.transform.scale(self.image, (int(180 * ESCALA_X), int(180 * ESCALA_Y)))
        self.rect = self.image.get_rect()
        self.rect.x = LARGURA
    ...
    if estado_jogo == TELA_INICIAL:
        ...
        for evento in pygame.event.get():
            if evento.type == QUIT:
                pygame.quit()
                exit()
            elif (evento.type == MOUSEBUTTONDOWN and evento.button == 1 and continuar_rect.collidepoint(evento.pos)) \
              or (evento.type == KEYDOWN and evento.key == K_RETURN):
                estado_jogo = JOGANDO
